...
def analysis_file(filename, metric='ig2_gold_gap'):
    ...
    # get imp pos by rel_ig2
    pos_cnt_rel = Counter()
    for bn_bag in bn_bag_list:
        for bn in bn_bag:
            pos_cnt_rel.update([pos_list2str(bn)])
    bn_rel = parse_bn(pos_cnt_rel, len(bn_bag_list), mode_ratio_rel)  # 所有bag, 也就是当前relation, 所以用的是rel ratio
...
for filename in os.listdir(rlts_dir):
    if filename.endswith('.rlt.jsonl') and 'filtered-gap-rm-base' in filename:
        ...
        for max_it in range(6):
            ave_bn_num, bn_bag_list, bn_rel = analysis_file(filename)  # ig2_gold_gap
            if ave_bn_num < 2:
                mode_ratio_bag -= 0.05
            if ave_bn_num > 5:
                mode_ratio_bag += 0.05
