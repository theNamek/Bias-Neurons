...
bn_dir = '../bias_component_results_ori_v1/bn/'
fig_dir = '../bias_component_results_ori_v1/figs/'
...
tot_rel_num = 0
tot_bneurons = 0
bn_bag_counter = Counter()
for filename in os.listdir(bn_dir):
    if not filename.startswith('bn_bag-'):
        continue
    with open(os.path.join(bn_dir, filename), 'r') as f:
        bn_bag_list = json.load(f)
        for bn_bag in bn_bag_list:
            for bn in bn_bag:
                bn_bag_counter.update([bn[0]])
                y_points.append(bn[0])
        tot_bag_num += len(bn_bag_list)
for k, v in bn_bag_counter.items():
    print('bn_bag_counter k v', k, v)
    tot_bneurons += bn_bag_counter[k]
for k, v in bn_bag_counter.items():
    bn_bag_counter[k] /= tot_bneurons

# average # Bneurons
print('average ig2_bn', tot_bneurons / tot_bag_num)
...
# =========== plot bias neuron distribution ===========

plt.figure(figsize=(8, 3))

x = np.array([i + 1 for i in range(12)])
y = np.array([bn_bag_counter[i] for i in range(12)])
plt.xlabel('Layer', fontsize=20)
plt.ylabel('Percentage', fontsize=20)
plt.xticks([i + 1 for i in range(12)], labels=[i + 1 for i in range(12)], fontsize=20)
plt.yticks(np.arange(-0.8, 1.0, 0.2), labels=[f'{np.abs(i)}%' for i in range(-40, 50, 10)], fontsize=5)
...
plt.savefig(os.path.join(fig_dir, 'bneurons_distribution.pdf'), dpi=100)
plt.close()

# ========================================================================================
#                       bias neuron intersection analysis
# ========================================================================================
...
print(f'ig2 bn has on average {inner_ave_intersec} inner bn interseciton')
...
for rel, bn_bag_list in bn_bag_list_per_rel.items():
    ...
    for i in range(0, len_bn_bag_list):
        for j in range(0, 100):
            bn_bag_1 = bn_bag_list[i]
            # print(bn_bag_list_per_rel.keys())
            # print('rel', rel)
...
print(f'ig2 bn has on average {inter_ave_intersec} inter bn interseciton')
