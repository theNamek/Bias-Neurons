...
fig_dir = '../bias_component_results_ori_v2/gender-neg/figs/'
...
plt.savefig(os.path.join(fig_dir, 'bneurons_distribution_bias.pdf'), dpi=100)
plt.close()
...
for filename in os.listdir(bn_dir):
    ...
    with open(os.path.join(bn_dir, filename), 'r') as f:
        bn_bag_list = json.load(f)
    rel = filename.split('.')[0].split('bag-')[1]
    print('rel', rel)
